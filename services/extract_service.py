...
def get_boxes(file):
  ...
  with pdfplumber.open(f, laparams = { "line_overlap": 0.7, "all_texts": True }) as pdf:
    for page in pdf.pages:
      ...
      doc_name = 'renta'
      presets = document_coords['{!s}_presets'.format(doc_name)][version]
      for key, value in document_coords[doc_name][version].items():
        parser = RowParser(get_row_parser_method_name(key))
        for coords in value:
          crop_coords = calculate_coords(coords, key, presets)
